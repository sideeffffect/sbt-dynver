...
object DynVerPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val dynverInstance =
      settingKey[DynVer]("The dynver instance for this build")
    val dynverCurrentDate =
      settingKey[Date]("The current date, for dynver purposes")
    ...
    val dynverVTagPrefix =
      settingKey[Boolean]("Whether or not tags have a 'v' prefix")
    val dynverCheckVersion =
      taskKey[Boolean]("Checks if version and dynver match")
    val dynverAssertVersion =
      taskKey[Unit]("Asserts if version and dynver match")
    val dynverTagPrefix =
      settingKey[String]("The prefix to use when matching the verision tag")
...
final case class GitDescribeOutput(ref: GitRef,
  ...
  def version(sep: String): String = {
    ...
    if (isCleanAfterTag)
      ref.dropV.value + dirtySuffix // no commit info if clean after tag
...
object GitDescribeOutput
  ...
  private[sbtdynver] final class Parser(vTagPrefix: Boolean) {
    ...
    private def parse0(ref: String,
                       ...
                       dirty: String) = {
      val commit =
        if (dist == null || sha == null) GitCommitSuffix(0, "")
        else GitCommitSuffix(dist.toInt, sha)
      GitDescribeOutput(
        GitRef(ref),
        commit,
  ...
  private[sbtdynver] def timestamp(d: Date): String =
    "%1$tY%1$tm%1$td-%1$tH%1$tM" format d
  private[sbtdynver] def fallback(separator: String, d: Date) =
    s"HEAD$separator${timestamp(d)}"
}
...
sealed case class DynVer(wd: Option[File],
  ...
  def isVersionStable(): Boolean =
    getGitDescribeOutput(new Date).isVersionStable
  ...
  def getGitDescribeOutput(d: Date): Option[GitDescribeOutput] = {
    ...
    Try(process !! impl.NoProcessLogger).toOption
      ...
      .flatMap(
        output =>
          if (output.hasNoTags)
            getDistanceToFirstCommit().map(
              dist =>
                output.copy(
                  commitSuffix = output.commitSuffix.copy(distance = dist)
              )
            )
          else Some(output)
      )
